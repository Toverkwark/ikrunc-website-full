...
`perl scripts/FilterAndSelect.pl -i $QualitiesFileLocation/$RefSeq.qualities.4 -o temp/$timestamp -c 1 -e 50 -n -1 -r $RefSeq -s $Species`;
...
my $InputFile="temp/" . $timestamp . ".seq";
open (IN, $InputFile) or die "ERROR in $ScriptName: Cannot open inputfile $InputFile\n";
open (OUT, ">", "temp/" . $timestamp . ".svg") or die "ERROR in $ScriptName: Cannot create svg file\n";
open (OUTHTML, ">", "temp/" . $timestamp . ".svg.html") or die "ERROR in $ScriptName: Cannot create svg html file\n";

#First read in all intron/exon information of the gene we're plotting
#TODO: Change into grep -P
